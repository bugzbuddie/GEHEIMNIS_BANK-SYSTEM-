...
class WithdrawBank extends WithdrawAccount {
...
(function () {
  const bank = new WithdrawBank({ ACCOUNT, ELEMENT: document });
  bank.init();
  
  var WITHDRAW_BUTTON = document.getElementById("withdraw-btn");
  
  WITHDRAW_BUTTON.disabled = false;
  
  var WIDTH = 0;
  var PROCESS = document.getElementById("withdraw-process");
  var PROGRESS = document.getElementById("progress");
  var PROGRESS_BAR = document.getElementById("progress-bar");
  var PROGRESS_INDICATOR = document.getElementById("indicator");
  
  WITHDRAW_BUTTON.addEventListener("click", () => {
    const AMOUNT = document.getElementById("withdraw-balance").value;
    ...
    if (!CARD) {
      this.WITHDRAW_ERROR.innerText = "You must choose a card";
    } else if (BALANCE == 0 || BALANCE == null) {
      this.WITHDRAW_ERROR.innerText = "Inefficient balance";
    } else if (AMOUNT > BALANCE) {
      this.WITHDRAW_ERROR.innerText = "Amount must not be greater than your balance";
    } else if (AMOUNT < 10) {
      this.WITHDRAW_ERROR.innerText = "Amount must be grater than or equal to 10";
    } else {
      WITHDRAW_BUTTON.disabled = true;
      this.WITHDRAW_ERROR.innerText = "";
      ...
      var _DATA = {
        ...
        ACCOUNT_BALANCE: BALANCE - AMOUNT_OR_VALUE,
